...
export default function TestNamePage() {
  ...
  const [rows, setRows] = useState(INITIAL_TEST_NAME_ROWS);
...
const nextGroupIdRef = useRef(
  Math.max(...INITIAL_TEST_NAME_ROWS.map((row) => row.groupId)) + 1
);
...
const handleDeleteRow = useCallback(
  ...
  if (!testName) {
    setFieldErrors({ 
      testName: !testName ? 'Test Name is required.' : '', 
    });
    return;
  }
 ...
 tid: nextTcidRef.current,
    ...
    if (editingRowId) {
      setRows((current) =>
        current.map((row) =>
         row.id === editingRowId
  ? { ...row, groupName: rowPayload.groupName, subGroupName: rowPayload.subGroupName, testName: rowPayload.testName, medicalName: rowPayload.medicalName, standardName: rowPayload.standardName, fee: rowPayload.fee }
  : row
        ),
      );
      setIsComponentModalOpen(false);
...
const filteredRows = useMemo(() => {
  ...
  { title: 'Group Name', dataIndex: 'groupName', key: 'groupName', width: 130, className: 'test-name-col-group-name' },
