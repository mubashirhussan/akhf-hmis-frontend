export const HOSPITAL_SERVICES_INITIAL_VALUES = {
  currentAmount: null,
...
export function getHospitalServicesFields({ adjustMode, showCurrentAmount } = {}) {
  const fields = [];
  ...
  if (showCurrentAmount) {
    fields.push({
      ...
      props: { disabled: true, className: 'hospital-services-percentage-input' },
    });
  }
  ...
  if (adjustMode === 'percentage') {
    fields.push({
      ...
      props: { min: 0.01, max: 100, className: 'hospital-services-percentage-input' },
    });
  }
  ...
  if (adjustMode === 'fixed') {
    fields.push({
      ...
      props: { min: 1, className: 'hospital-services-percentage-input' },
    });
  }
